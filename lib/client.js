...
Client.prototype.connect = function () {
    ...
    zk.once('init', function (broker) {
        ...
        for (var b in broker) {
            /**
             * broker[b].host = (os.platform().match(/^win*i) === true  && broker[b].host === '0.0.0.0') ? '127.0.0.1' : broker[b].host
             * Adding this change to make this module work on windows and vagrant. Vagrant setup has this
             * requirement to resolve to host 0.0.0.0 for forwarded ports. Windows don't work with node.js's net module
             * to resolve to host 0.0.0.0 for broker ports.
             * @type {string}
             */
            var os = require('os');
            broker[b].host = (/^win*/i.test(os.platform()) === true  && broker[b].host === '0.0.0.0') ? '127.0.0.1' : broker[b].host ;
